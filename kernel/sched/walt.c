...
unsigned int sysctl_sched_dynamic_ravg_window_enable = (HZ == 100);
				...
				loff_t *ppos)
{
	...
	if (write && (HZ != 100 || !sysctl_sched_dynamic_ravg_window_enable))
		goto unlock;
...
{
	if (HZ == 100 && sysctl_sched_dynamic_ravg_window_enable) {
		if (fps > FPS90)
...
unsigned int sysctl_sched_capacity_margin_down[MAX_MARGIN_LEVELS] = {
			[0 ... MAX_MARGIN_LEVELS-1] = 1575}; /* ~15% margin */
